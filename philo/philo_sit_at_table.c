...
/*   Updated: 2023/12/17 16:29:33 by fporciel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
/*
...
static int	phi_init_internal_checkers(t_name *head, t_philo *phi)
{
	...
	while ((tmp && (tmp != head)) || (i == 0))
	{
		...
		if (pthread_mutex_init(&(tmp->killer), NULL) != 0)
			return (phi_bad_checking(phi, tmp, 1));
		if (pthread_mutex_init(&(tmp->over), NULL) != 0)
			return (phi_bad_checking(phi, tmp, 2));
		tmp->i = 0;
		tmp->iseating = 0;
		tmp->issleeping = 0;
		tmp->isthinking = 0;
